...
const ShopViews: React.FC<ShopViewsProps> = ({
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (userInfo?.userId && fetchedReviews.length > 0) {
          const firstReview = fetchedReviews.find(
            (r: any) => (r.customerId?._id || r.customerId) === userInfo.userId,
          );
          setUserReview(firstReview || null);

          // const firstReview = fetchedReviews[0];
          const reviewUserId =
            firstReview.customerId?._id || firstReview.customerId;
  ...
  const handleSaveImage = async (images: File[] | null) => {
    try {
    ...
    const imageDataArray = await Promise.all(uploadPromises);
        ...
        let response = await addImages(imageDataArray);
                    ...
                    <div className="glass-card rounded-2xl p-6 text-center">
                      ...
                      <div className="flex justify-center gap-1 mb-2 text-yellow-400">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            className={`w-5 h-5 ${s <= Number(avgRating) ? "fill-current" : "text-gray-300 dark:text-gray-600"}`}
                          />
                        ))}
                      </div>
                          ...
                          initialValues={{
                            rating:
                              isEditing && userReview
                                ...
                                : 0,
                            comment:
                              isEditing && userReview ? userReview.comment : "",
                          }}
                          enableReinitialize
                              ...
                              <div className="flex items-center gap-4">
                                <div className="flex text-yellow-400">
                                  {[...Array(5)].map((_, i) => (
                                    <Star
                                      key={i}
                                      className={`w-4 h-4 ${i < Number(r.rating) ? "fill-current" : "text-gray-200"}`}
                                    />
                                  ))}
                                </div>
                                {!isVendor && isMyReview && (
