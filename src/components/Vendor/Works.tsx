...
import { Search, MapPin, Calendar, DollarSign, Filter, ArrowRight, Briefcase } from "lucide-react";
...
const Works: React.FC = () => {
  ...
  const filteredWorks = DUMMY_WORKS.filter((work) => {
    const matchesSearch =
      work.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      work.description.toLowerCase().includes(searchTerm.toLowerCase());

    const matchesLocation = work.location.toLowerCase().includes(locationSearchTerm.toLowerCase());
    
    const matchesFilter = filterType === "All" || work.serviceType === filterType;

    return matchesSearch && matchesLocation && matchesFilter;
  });

  const serviceTypes = ["All", ...Array.from(new Set(DUMMY_WORKS.map(w => w.serviceType)))];
    ...
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredWorks.map((work) => (
          <div
            key={work._id}
            className="group bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-300 flex flex-col justify-between relative overflow-hidden"
          >
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-auto">
              <div className="flex flex-col">
                 <span className="text-xs text-gray-400 font-medium font-sans">Budget</span>
                 <div className="flex items-center text-lg font-bold text-gray-900 font-sans">
                    <DollarSign className="w-4 h-4 text-green-500 mr-0.5" />
                    {work.budget}
                 </div>
              </div>
              
              <Button className="bg-blue-600 text-white rounded-xl px-5 py-2 hover:bg-blue-700 transition-colors flex items-center gap-2 shadow-md shadow-blue-200">
                Apply Now
        ...
        {filteredWorks.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900">No works found</h3>
