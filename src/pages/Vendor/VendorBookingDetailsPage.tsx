    ...
    <div className="min-h-screen bg-gray-50/50 p-6 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <motion.div
          ...
          className={`${statusConfig.bg} border-2 border-${
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              <div
                className={`p-3 rounded-xl ${statusConfig.bg} ${statusConfig.color} border-2 border-current`}
              >
                {statusConfig.icon}
              </div>
              <div>
                ...
                <p className={`text-xl font-black ${statusConfig.color}`}>
                  {statusConfig.label}
                </p>
              </div>
            </div>
            ...
            {bookingData.paymentStatus == "paid" &&
              ...
              bookingData.paymentMethod != "payAtShop" && (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              ...
              <div className="space-y-4">
                {/* Added flex justify-between to push icon to the right */}
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className="h-14 w-14 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-blue-600 font-bold text-lg border-2 border-white shadow-md">
                      {bookingData.customerId?.name?.charAt(0).toUpperCase() ||
                        "C"}
                    </div>
                  ...
                  {/* New Location Icon on the right */}
                  <div 
                    onClick={()=>GoogleMapDirection(bookingData?.address.coordinates.lat,bookingData?.address.coordinates.lng)}
                  className="p-2 bg-gray-50 rounded-lg hover:bg-gray-200 cursor-pointer transition-colors border border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 bg-purple-50/50 rounded-xl border border-purple-100">
                  ...
                  <p className="text-base font-bold text-gray-900">
                    {bookingData.date
                      ? format(new Date(bookingData.date), "MMM dd, yyyy")
                      : "N/A"}
                  </p>
                </div>
              ...
              >
                ...
                <div className="p-4 bg-rose-50/50 rounded-xl border border-rose-100">
                  <p className="text-sm text-gray-700 leading-relaxed">
                    {bookingData.addressId.street}, {bookingData.addressId.city}
                    , {bookingData.addressId.state} -{" "}
                    {bookingData.addressId.pincode}
                  </p>
                </div>
              </motion.div>
            )}
                ...
                <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${
                      PAYMENT_STATUS_CONFIG[bookingData.paymentStatus]?.bg
                    } ${
                      PAYMENT_STATUS_CONFIG[bookingData.paymentStatus]?.color
                    }`}
                  >
                    {PAYMENT_STATUS_CONFIG[bookingData.paymentStatus]?.label ||
