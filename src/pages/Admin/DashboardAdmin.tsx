import React, { useEffect, useState } from "react";
import {
  Card,
  CardHeader,
  CardTitle,
  CardContent,
  CardDescription,
} from "../../components/ui/card";
import {
  Users,
  Store,
  ShoppingCart,
  DollarSign,
  Clock,
} from "lucide-react";
import { adminDashbordData } from "../../Services/AdminApiService";

const DashboardAdmin = () => {

    const [totalUsers,setTotalUsers] = useState<number>(0)
    const [totalVendors,setTotalVendors] = useState<number>(0)
    const [totalCustomers,setToalCustomer] = useState<number>(0)
    const [totalRequestPendings,setTotalRequestPendings] = useState<number> (0)
    const [totalRejectedVendors,setTotalRejectedVendors] = useState<number>(0)
    const [totalVerifiedVendors,setTotalVerifiedVendors] = useState<number>(0)
    useEffect(()=>{
        dashboardDocument()
    },[])




    const  dashboardDocument= async () =>{
        try{

            let response = await adminDashbordData()

            if(response.data){
                setTotalVendors(response.data.totalVednors)
                setToalCustomer(response.data.totalCutomers)
                setTotalUsers(response.data.totalVednors+response.data.totalCutomers)
                setTotalRequestPendings(response.data.pendingVendors)
                setTotalRejectedVendors(response.data.rejectedVendors)
                setTotalVerifiedVendors(response.data.verifiedVendors)


            }
            

        }catch(error:unknown){



        }
        
    }


  const stats = [
  {
    title: "Total Users",
    value: totalUsers,
    icon: <Users className="text-blue-600" size={18} />,
    description: "All registered users",
    iconBg: "bg-blue-100",
  },
  {
    title: "Total Vendors",
    value: totalVendors,
    icon: <Store className="text-green-600" size={18} />,
    description: "All business vendors",
    iconBg: "bg-green-100",
  },
  {
    title: "Total Customers",
    value: totalCustomers,
    icon: <ShoppingCart className="text-purple-600" size={18} />,
    description: "Active customer profiles",
    iconBg: "bg-purple-100",
  },
  {
    title: "Requests Pending",
    value: totalRequestPendings,
    icon: <Clock className="text-orange-600" size={18} />,
    description: "Awaiting admin approval",
    iconBg: "bg-orange-100",
  },
  {
    title: "Verified Vendors",
    value: totalVerifiedVendors,
    icon: <Store className="text-emerald-600" size={18} />,
    description: "Approved and verified vendors",
    iconBg: "bg-emerald-100",
  },
  {
    title: "Rejected Vendors",
    value: totalRejectedVendors,
    icon: <Store className="text-red-600" size={18} />,
    description: "Rejected vendor applications",
    iconBg: "bg-red-100",
  },
  {
    title: "Total Revenue",
    value: "$54,800",
    icon: <DollarSign className="text-yellow-600" size={18} />,
    description: "Revenue this month",
    iconBg: "bg-yellow-100",
  },
];


 return (
    <div className="min-h-screen bg-gray-100">
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-6 sm:py-10">
        {/* Responsive Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {stats.map((item, index) => (
            <Card
              key={index}
              className="bg-white shadow-sm hover:shadow-md transition-all duration-200 rounded-xl border border-gray-200 p-4 flex flex-col justify-between"
            >
              <CardHeader className="flex items-center justify-between p-0 mb-1">
                <CardTitle className="text-sm sm:text-base font-medium text-gray-500">
                  {item.title}
                </CardTitle>
                <span
                  className={`${item.iconBg} p-1.5 sm:p-2 rounded-md flex items-center justify-center`}
                >
                  {item.icon}
                </span>
              </CardHeader>

              <CardContent className="p-0">
                <p className="text-lg sm:text-xl font-semibold text-gray-900 leading-tight">
                  {item.value}
                </p>
                <CardDescription className="text-xs sm:text-sm text-gray-500 mt-0.5">
                  {item.description}
                </CardDescription>
              </CardContent>
            </Card>
          ))}
        </div>
      </main>
    </div>
  );
};

export default DashboardAdmin;
